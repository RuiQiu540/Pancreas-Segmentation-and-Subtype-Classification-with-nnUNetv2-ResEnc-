...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        if isfile(csv_path):
            with open(csv_path, newline='') as f:
                ...
                for row in reader:
                    key = (row.get('case_id') or row.get('Names') or '').strip()
                    if key.endswith('.nii.gz'):
                        key = key[:-7]
                    if key:
                        self.case2subtype[key] = int(row.get('subtype') or row.get('Subtype'))
            self.print_to_log_file(f"Loaded {len(self.case2subtype)} classification labels from case_to_subtype.csv")
        else:
            self.print_to_log_file("WARNING: case_to_subtype.csv not found - classification loss will be skipped")
        ...
        if not self.was_initialized:
            ...
            self.network = self.build_network_architecture(
                ...
                self.enable_deep_supervision
                        ...
                        # === Classification head: from highest-res segmentation logits ===
            self.cls_head = nn.Linear(self.label_manager.num_segmentation_heads, self.num_subtypes).to(self.device)
    ...
    def _norm_case_id(self, s: str) -> str:
        s = os.path.basename(str(s))
        if s.endswith('.nii.gz'):
            s = s[:-7]
        # remove modality channel suffix like _0000 / _0001
        if len(s) > 5 and s[-5] == '_' and s[-4:].isdigit():
            s = s[:-5]
        return s
    def _save_cls_head_file(self, tag: str):
        if hasattr(self, 'cls_head'):
            try:
                import torch, os
                from os.path import join
                out = join(self.output_folder, f'cls_head_{tag}.pth')
                torch.save(self.cls_head.state_dict(), out)
                self.print_to_log_file(f"[cls] saved head -> {out}")
            except Exception as e:
                self.print_to_log_file(f"[cls] WARN: failed to save cls_head: {e}")
    ...
    def on_train_end(self):
        ...
        self._save_cls_head_file('final')
        self.current_epoch += 1
            ...
            if self.case2subtype:
                ...
                if keys is not None:
                    ...
                    for idx, k in enumerate(keys):
                        ...
                        k = self._norm_case_id(k)
                        t = self.case2subtype.get(k, None)
                        if t is not None:
                            keep_idx.append(idx)
                    ...
                    if len(keep_idx) > 0:
                        ...
                        self.cls_losses.append(float(cls_loss.item()))
                    else:
                        self.print_to_log_file(f"[cls] used=0/{len(keys)} (no labelled samples in this batch)")
    ...
    def on_train_epoch_end(self, train_outputs: List[dict]):
        ...
        if self.cls_predictions and self.cls_targets:
            ...
            self.logger.log('cls_macro_f1', float(macro_f1), self.current_epoch)
            self.print_to_log_file(
                f"Epoch {self.current_epoch} - Classification: Loss={mean_cls_loss:.4f}, Macro F1={macro_f1:.4f}",
                also_print_to_console=True
            )
        else:
            self.print_to_log_file(
                f"[Cls][Train] epoch={self.current_epoch} no samples used for classification (check name mapping/CSV)",
                also_print_to_console=True
            )
        ...
        if self.local_rank == 0:
            if not self.disable_checkpointing:
                ...
                checkpoint = {
                    ...
                    'cls_head_weights': self.cls_head.state_dict() if hasattr(self, 'cls_head') else None,
    ...
    def load_checkpoint(self, filename_or_checkpoint: Union[dict, str]) -> None:
        ...
        if self.is_ddp:
            ...
            if hasattr(self, 'cls_head') and ('cls_head_weights' in checkpoint) and (checkpoint['cls_head_weights'] is not None):
                try:
                    self.cls_head.load_state_dict(checkpoint['cls_head_weights'])
                except Exception as e:
                    self.print_to_log_file(f"[cls] WARN: failed to load cls_head_weights in DDP: {e}")
        else:
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        if self.is_ddp and self.batch_size == 1 and self.enable_deep_supervision and self._do_i_compile():
            self.print_to_log_file(
                ...
                "If you encounter crashes in validation then this is because torch.compile forgets "
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for i, k in enumerate(dataset_val.identifiers):
                ...
                data = data[:]  # blosc2 -> numpy

                if self.is_cascaded:
                    seg_prev = seg_prev[:]
                    data = np.vstack((
                        data,
                        convert_labelmap_to_one_hot(seg_prev, self.label_manager.foreground_labels,
                                                    output_dtype=data.dtype)
                    ))

                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")  # ignore 'writable' warning
                ...
                if prediction.ndim == 4:
                    ...
                    pred_logits = prediction  # already 5D
        ...
        try:
            ...
            with open(csv_out, 'w', newline='') as f:
                ...
                for n, s in zip(pred_names, pred_subtypes):
                    w.writerow([n, s])
            self.print_to_log_file(f"Saved classification CSV -> {csv_out}")
        except Exception as e:
            self.print_to_log_file(f"Failed to save classification CSV: {e}")
