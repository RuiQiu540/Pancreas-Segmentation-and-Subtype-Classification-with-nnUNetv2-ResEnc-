...
class nnUNetPredictor(object):
    def __init__(self,
        ...
        self.perform_everything_on_device = perform_everything_on_device
                # ---- classification head params (optional) ----
        self.cls_head_w = None   # numpy array, shape [C_out, C_in]
        self.cls_head_b = None   # numpy array, shape [C_out]

                # --- classification aggregation (added) ---
        self._tmp_cls_logits_tta = None        # current patch TTA average cls logits [B, C]
        self._last_case_cls_logits = None      # current case cls logits after SW+TTA [C]
        self._current_case_cls_logits = None   # (optional) fold-averaged cls logits [C]

    def _split_seg_and_cls(self, out):
        # Split outputs into (segmentation, classification).
        # If cls has spatial dims, pool to [B, C].
        seg, cls = None, None
        if isinstance(out, tuple):
            ...
            seg = out.get('seg', out.get('logits', None))
            cls = out.get('cls', None)
        else:
            seg = out
        if cls is not None and getattr(cls, 'ndim', 0) > 2:   # [B,C,H,W] -> [B,C]
            import torch.nn.functional as F
            cls = F.adaptive_avg_pool2d(cls, 1).flatten(1)
        return seg, cls
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        # added            # ---- extract classification head weights (if present) ----
                ...
                if w is None:
                    for container in ('state_dict', None):
                        ...
                        if isinstance(sd, dict):
                            for kw, kb in [
                                ('cls_head_weights.weight', 'cls_head_weights.bias'),
                                ('cls_head.weight', 'cls_head.bias'),
                                ('module.cls_head.weight', 'module.cls_head.bias'),
                            ]:
                                if kw in sd:
                                    w, b = sd.get(kw, None), sd.get(kb, None)
                                    break
                        if w is not None:
                            break
                if w is not None:
                    self.cls_head_w = w.detach().cpu().numpy()
                    self.cls_head_b = (b.detach().cpu().numpy() if b is not None
                                    else np.zeros((self.cls_head_w.shape[0],), dtype=np.float32))
                    W = self.cls_head_w
                    bb = self.cls_head_b if self.cls_head_b is not None else np.zeros((W.shape[0],), dtype=np.float32)
                    print(f"[classification] loaded head: W{W.shape} b{bb.shape} | "
                        f"W[min,max,norm]=({W.min():.4g},{W.max():.4g},{np.linalg.norm(W):.4g}) "
                        f"b[min,max]=({bb.min():.4g},{bb.max():.4g})")
                else:
                    print("[classification] WARNING: cls head NOT found in checkpoint; will fallback.")
    ...
    def predict_from_data_iterator(self,
        ...
        """
                # ---- classification CSV & F1 prep ----
        subtype_csv_path = None
        y_true, y_pred = [], []
                # ---- classification CSV persistent path (init once, write header once) ----
        subtype_csv_path = None  # will be set on first case when we know out_dir
        default_out_dir = getattr(self, "_out_dir", None)
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                prediction_t = self.predict_logits_from_preprocessed_data(data)
                prediction    = prediction_t.float().cpu().detach().numpy()   # ← Part B：确保是 FP32 的 numpy

                        # ---------- [add] CSV & classification with guard ----------

                pred_subtype = None   # placehold, avoid NameError when exception occurs

                try:
                    # 1) only create CSV + header on first case
                    if subtype_csv_path is None:
                        out_dir = os.path.dirname(ofile) if ofile is not None else default_out_dir
                        if out_dir is not None:
                            os.makedirs(out_dir, exist_ok=True)
                            subtype_csv_path = os.path.join(out_dir, "subtype_results.csv")
                            # clean old file (optional)
                            if os.path.exists(subtype_csv_path):
                                try:
                                    os.remove(subtype_csv_path)
                                except Exception:
                                    pass
                            with open(subtype_csv_path, "w", newline="") as fcsv:
                                wr = csv.writer(fcsv); wr.writerow(["Names", "Subtype"])
                        elif self.verbose:
                            print("[classification] WARNING: cannot infer output dir; CSV will not be written yet.")

                    # 2) case name (same as segmentation export)
                    file_ending = self.dataset_json.get('file_ending', '.nii.gz')
                    if ofile is not None:
                        case_name = os.path.basename(ofile) + file_ending
                    else:
                        src0 = properties.get('list_of_data_files', [None])[0]
                        case_name = (os.path.basename(src0) if src0 is not None else "unknown_case") + file_ending

                    # 3) normalize prediction shape and compute v (mean probability per channel)
                    logits = prediction
                    ...
                    b = self.cls_head_b if getattr(self, "cls_head_b", None) is not None else 0.0
                    pred_subtype = None

                    if W is not None and W.shape[1] == pooled.shape[0]:
                        cls_logit = W.dot(pooled) + b         # [3]
                        pred_subtype = int(np.argmax(cls_logit))
                        if self.verbose:
                    ...
                    # 5) only append to CSV if pred_subtype is successfully computed
                    if (subtype_csv_path is not None) and (pred_subtype is not None):
                        with open(subtype_csv_path, "a", newline="") as fcsv:
                            wr = csv.writer(fcsv); wr.writerow([case_name, pred_subtype])

                except Exception as e:
                    # print full stack for debugging
                    print("[classification] ERROR during CSV/pred:", e)
                    traceback.print_exc()
                    # don't raise, let segmentation export continue

                # build case name (must include file ending, e.g. .nii.gz)
                # ---------- per-case classification (append one line) ----------
                # assemble case name (must include file ending, e.g. .nii.gz)
        ...
        empty_cache(self.device)
                # ---- optional macro-F1 report ----
        if len(y_true) > 0:
            try:
                from sklearn.metrics import f1_score, classification_report
                macro_f1 = f1_score(y_true, y_pred, average='macro')
                print(f"[classification] Macro-F1 on provided GT: {macro_f1:.4f}")
                # write a report to the output directory (same as CSV)
                if subtype_csv_path is not None:
                    rep_path = os.path.join(os.path.dirname(subtype_csv_path), "classification_report.txt")
                    with open(rep_path, "w", encoding="utf-8") as f:
                        f.write(f"Macro-F1: {macro_f1:.6f}\n")
                        f.write(classification_report(y_true, y_pred, digits=4))
            except Exception as e:
                print("[classification] Failed to compute macro-F1:", e)
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                seg_i, cls_i = self._split_seg_and_cls(self.network(torch.flip(x, axes)))
                seg_i = torch.flip(seg_i, axes)
                prediction += seg_i
                nviews += 1
                if cls_i is not None:
                    cls_acc = cls_i if cls_acc is None else (cls_acc + cls_i)
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        try:
            ...
            if isinstance(ss, (tuple, list)):
                sy = int(round(ph * ss[0]))
                sx = int(round(pw * ss[1]))
            else:
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor) \
        ...
        with (torch.autocast('cuda', dtype=torch.float16, enabled=True) if self.device.type == 'cuda' else dummy_context()):
            ...
            slicers = self._internal_get_sliding_window_slicers(data.shape[1:])
                        # --- compute pixel step from step_size ---
            patch_h, patch_w = self.configuration_manager.patch_size[:2]
            # step_size may be float or tuple during inference
            ss = getattr(self, "inference_step_size", (0.5, 0.5))



            if self.perform_everything_on_device and self.device.type != 'cpu':
...
def predict_entry_point_modelfolder():
    ...
    predictor.initialize_from_trained_model_folder(args.m, args.f, args.chk)
        # let data_iterator know output directory when ofile=None
    predictor._out_dir = args.o
...
def predict_entry_point():
    ...
    parser.add_argument('--disable_progress_bar', action='store_true', required=False, default=False,
                        help='Set this flag to disable progress bar. Recommended for HPC environments (non interactive '
                             'jobs)')
    parser.add_argument('--cls_gt_csv', type=str, required=False, default=None,
                        help='Optional: CSV with columns Names,Subtype for macro-F1 evaluation.')
    ...
    predictor._out_dir = args.o
        ...
        # ---- optional ground-truth mapping for classification F1 ----
    predictor._cls_gt_map = None
    if args.cls_gt_csv is not None and os.path.isfile(args.cls_gt_csv):
        gt_map = {}
        import csv as _csv
        with open(args.cls_gt_csv, 'r', newline='') as f:
            ...
            if name_col and subtype_col:
                for row in reader:
                    nm = os.path.basename(row[name_col])
                    gt_map[nm] = int(row[subtype_col])
        predictor._cls_gt_map = gt_map
        if predictor.verbose:
            print(f"[classification] loaded GT for {len(gt_map)} cases from {args.cls_gt_csv}")
